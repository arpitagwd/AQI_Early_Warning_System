...
# from live_api  import (fetch_current_aqi, fetch_historical_aqi, 
#                         get_city_list, fetch_recent_aqi_values)
...
def build_feature_vector(city, pollutants_dict, month, le_city, le_season,
                          hist_aqi, live_hist_aqi=None):
    """
    Build one row of features for the classifier.
    
    hist_aqi      : list of AQI values from Kaggle CSV  — always available as fallback
    live_hist_aqi : list of AQI values from OWM API     — used when available in live mode
    """
    season = SEASON_MAP[month]
        # city_enc   = int(le_city.transform([city])[0])
    # Safe encoding — if city not in training data, find nearest known city
...
# ── Choose data source for lag features ─────────────────────
    # Live API data reflects actual recent AQI — use it when available
    # Fall back to Kaggle 2020 CSV when API data is unavailable
    if live_hist_aqi and len(live_hist_aqi) >= 7:
        aqi_source   = live_hist_aqi
        source_label = "live API (real recent data)"
    else:
        aqi_source   = hist_aqi
        source_label = "Kaggle CSV (2020 baseline)"

    print(f"[build_feature_vector] Lag source : {source_label}")
    print(f"[build_feature_vector] Last 7 vals: "
          f"{[round(v, 1) for v in list(aqi_source)[-7:]]}")

    aqi_vals = list(aqi_source)[-7:]
    while len(aqi_vals) < 7:
        aqi_vals.insert(0, aqi_vals[0] if aqi_vals else 100)
...
# If city has no historical data (e.g. Mumbai), use a nearby city's history
    ...
    # ── Fetch live historical AQI for lag features ───────────────
    live_hist_aqi = None

    if data_mode == "Live (OpenWeatherMap API)" and api_key:
            with st.spinner("Fetching recent AQI history for lag features..."):
                live_hist_aqi = fetch_recent_aqi_values(
                city, days=30, api_key=api_key
            )

    if live_hist_aqi and len(live_hist_aqi) >= 7:
            st.success(
                f"✅ Live lag features loaded — "
                f"last {len(live_hist_aqi)} days of real AQI used "
                f"(most recent: {live_hist_aqi[-1]:.0f} AQI)"
            )
    else:
            st.warning(
                "⚠️ Could not fetch live AQI history. "
                "Using 2020 Kaggle baseline for lag features."
            )
            live_hist_aqi = None
    ...
    feature_vec = build_feature_vector(
        city, poll_values, month,
        le_city, le_season,
        hist_aqi,       # Kaggle CSV — always available as fallback
        live_hist_aqi   # OWM live  — None in manual mode
    )
    bucket_label, confidence, probs, classes, scaled_vec = predict_aqi_bucket(
        feature_vec, clf, scaler, le_target
    )
...
# Fetch live lag data for city2 as well
        live_hist_aqi2 = None
        if data_mode == "Live (OpenWeatherMap API)" and api_key:
            with st.spinner(f"Fetching recent AQI history for {city2}..."):
                live_hist_aqi2 = fetch_recent_aqi_values(
                    city2, days=30, api_key=api_key
                )

        fvec2 = build_feature_vector(
            city2, poll2, month,
            le_city, le_season,
            city2_aqi,      # Kaggle CSV fallback
            live_hist_aqi2  # Live lag data for city2
        )
        scaled2 = scaler.transform(fvec2)
        lstm_seq2 = np.repeat(scaled2, LOOKBACK, axis=0)
        forecast2 = forecast_7_days(lstm, scaler, aqi_scaler, lstm_seq2)
